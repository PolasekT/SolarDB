...
def timedelta_total_seconds(delta: datetime.timedelta) -> int:
    """ Calculate total number of seconds in given delta. """

    return int(delta.days * 24 * 60 * 60 + delta.seconds)


def date_earliest_date_time(date: datetime.date) -> datetime.datetime:
    """
    Transform provided date into the earliest possible
    date-time with the same date.

    :param date: Date to transform.

    :return: Returns the earliest date-time for that date.
    """
...
def date_latest_date_time(date: datetime.date) -> datetime.datetime:
    """
    Transform provided date into the latest possible
    date-time with the same date.

    :param date: Date to transform.

    :return: Returns the latest date-time for that date.
    """
...
def freq_delta(freq: str) -> datetime.timedelta:
    """
    Convert input frequency string into delta time.

    :param freq: String representing the frequency - e.g. "5min".

    :return: Returns delta time for given frequency - e.g. 5 minutes.
    """

    return pd.tseries.frequencies.to_offset(freq).delta


def freq_min(frequencies: [str]) -> str:
    """ Get minimal frequency from given list. """

    result = None
    for frequency in frequencies:
        if result is None or freq_delta(frequency) < freq_delta(result):
            result = frequency
    return result


def freq_ceil(date_time: datetime.datetime, freq: str) -> datetime.datetime:
    """ Round given date-time up based on provided frequency. """

    frequency_seconds = int(freq_delta(freq).total_seconds())
    date_time_seconds = int(date_time.timestamp())
    ceil_seconds = math.ceil(date_time_seconds / frequency_seconds) * frequency_seconds

    return datetime.datetime.fromtimestamp(ceil_seconds)


def freq_floor(date_time: datetime.datetime, freq: str) -> datetime.datetime:
    """ Round given date-time up based on provided frequency. """

    frequency_seconds = int(freq_delta(freq).total_seconds())
    date_time_seconds = int(date_time.timestamp())
    ceil_seconds = math.floor(date_time_seconds / frequency_seconds) * frequency_seconds
